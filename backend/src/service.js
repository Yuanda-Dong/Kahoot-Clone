...
import { quizQuestionPublicReturn, quizQuestionGetCorrectAnswers, quizQuestionGetDuration } from './custom';
...
export const quizLock = (callback) =>
  new Promise((resolve, reject) => {
...
export const assertOwnsQuiz = (email, quizId) =>
...
export const getQuizzesFromAdmin = (email) =>
  quizLock((resolve, reject) => {
    resolve(
      Object.keys(quizzes)
...
export const addQuiz = (name, email) =>
  quizLock((resolve, reject) => {
    if (name === undefined) {
...
export const updateQuiz = (quizId, questions, name, thumbnail) =>
  quizLock((resolve, reject) => {
    if (questions) {
...
export const advanceQuiz = (quizId) =>
  quizLock((resolve, reject) => {
...
const quizHasActiveSession = (quizId) =>
  Object.keys(sessions).filter((s) => sessions[s].quizId === quizId && sessions[s].active).length > 0;
    ...
    if (answerList === undefined || answerList.length === 0) {
      ...
      if (session.position === -1) {
        ...
        session.players[playerId].answers[session.position] = {
          ...
          correct:
            JSON.stringify(quizQuestionGetCorrectAnswers(session.questions[session.position]).sort()) ===
